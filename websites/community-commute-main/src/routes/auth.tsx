...
export const Route = createFileRoute("/auth")({
  ...
  head: () => ({
    meta: [
      { title: "Sign in — Omni-Commute" },
      { name: "description", content: "Access your Omni-Commute Circles." },
    ],
  }),
  component: AuthPage,
});
        ...
        <div className="glass-strong rounded-3xl p-8 shadow-elevated">
          <div className="flex gap-1 p-1 rounded-xl bg-white/[0.03] border border-white/5 mb-6">
            <button
              ...
              className={`flex-1 py-2 rounded-lg text-sm font-medium transition ${mode === "signin" ? "bg-gradient-primary text-primary-foreground glow-primary" : "text-muted-foreground"}`}
            >
              Sign in
            </button>
            <button
              onClick={() => setMode("signup")}
              className={`flex-1 py-2 rounded-lg text-sm font-medium transition ${mode === "signup" ? "bg-gradient-primary text-primary-foreground glow-primary" : "text-muted-foreground"}`}
            >
              Create account
            </button>
          </div>
          ...
          {confirmationNeeded ? (
            <div className="text-center space-y-4">
              ...
              <button
                ...
                className="w-full inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-gradient-primary text-primary-foreground font-medium glow-primary hover:opacity-95 transition"
              >
            ...
            <>
              <button
                ...
                className="w-full inline-flex items-center justify-center gap-3 px-4 py-3 rounded-xl border border-white/10 bg-white/[0.03] text-foreground font-medium hover:bg-white/[0.07] transition"
              >
              ...
              <div className="relative my-6">
                ...
                <div className="relative flex justify-center text-xs uppercase">
                  <span className="bg-black/60 backdrop-blur-sm px-3 text-muted-foreground">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-gradient-primary text-primary-foreground font-medium glow-primary hover:opacity-95 transition disabled:opacity-60"
                >
